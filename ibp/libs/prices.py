...
class PriceService(object):
    ...
    def _get_from_database(self, date_time):
        ...
        prices = DBPrice.select().where((DBPrice.instrument == self.instrument._db_instrument) & (
                DBPrice.datetime > start) & (DBPrice.datetime < end)).order_by(DBPrice.datetime.desc()).limit(1)
        if prices:
    ...
    def _get_from_yahoo(self, date_time, failed_tries=0):
        ...
        if failed_tries >= config.getint("price_fetch_max_allowed_tries"):
            log.warning("Refusing to fetch {} {} because of {} previous failed attempt. Returning 0.001".format(
